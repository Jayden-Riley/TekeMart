...
// Define the type for the mock data structure
interface MockData {
  phones: Product[];
  computers: Product[];
  electronics: Product[];
...
const SearchBar: React.FC = () => {
  ...
  const getCategory = (): keyof MockData => {
    ...
    if (location.pathname.includes("toys")) return "toys";
    if (location.pathname.includes("videoGames")) return "games";
    if (location.pathname.includes("petSupplies")) return "petSupplies";
    return "phones"; // Default to 'phones' if no category is found
  };
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (query.trim().length > 0) {
      ...
      if (activeCategory in mockData) {
        results = mockData[activeCategory].filter((item) =>
          item.title.toLowerCase().includes(query.toLowerCase())
        );
      }

      // If no match in the active category, search across all categories
      if (results.length === 0) {
        Object.values(mockData).forEach((category) => {
          results = results.concat(
            category.filter((item: Product) =>
              item.title.toLowerCase().includes(query.toLowerCase())
            )
          );
        });
  ...
  const handleSelectProduct = (id: string) => {
    let foundCategory: keyof MockData | null = null;

    // Find the category of the selected product
    for (const category in mockData) {
      if (
        mockData[category as keyof MockData].some(
          (product) => product._id === id
        )
      ) {
        foundCategory = category as keyof MockData;
        break;
      }
    }

    if (foundCategory) {
      navigate(`/dashboard/${foundCategory}/${id}`);
    } else {
      // Fallback to a default category if the product is not found
      navigate(`/dashboard/phones/${id}`);
    }
    ...
    <div className="relative">
      <form className="w-full">
        <input
          ...
          className="px-4 py-2 pl-10 bg-gray-300 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-orange-500 w-full sm:w-80 md:w-96 lg:w-full " // Adjust width on different screens
        />
      </form>

      {searchQuery.trim().length > 0 && filteredResults.length > 0 && (
        <ul className="absolute bg-white border border-gray-300 rounded-md mt-2 w-full max-h-40 overflow-auto shadow-lg sm:w-80 md:w-96 lg:w-full z-50">
          {/* Dropdown width adjusts with screen size */}
